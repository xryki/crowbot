...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        switch (subcommand) {
            ...
            case 'antilink':
                await this.toggleFeature(message, config, 'antiLink', args[1], prefix);
                break;
                
            case 'antispam':
                await this.configureAntiSpam(message, config, args.slice(), prefix);
                break;
                
            case 'antitoken':
                await this.configureAntiToken(message, config, args.slice(), prefix);
                break;
                
            case 'antiwebhook':
                await this.toggleFeature(message, config, 'antiWebhook', args[1], prefix);
                break;
                
            case 'antibot':
                await this.toggleFeature(message, config, 'antiBot', args[1], prefix);
                break;
                
            case 'antimassmention':
                await this.configureAntiMassMention(message, config, args.slice(), prefix);
                break;
                
            case 'anticaps':
                await this.configureAntiCaps(message, config, args.slice(), prefix);
                break;
                
            case 'antiinvite':
                await this.toggleFeature(message, config, 'antiInvite', args[1], prefix);
                break;
                
            case 'antiban':
                await this.configureAntiBan(message, config, args.slice(), prefix);
                break;
            ...
            case 'whitelist':
                await this.manageWhitelist(message, config, args.slice(), prefix);
                break;
    ...
    async toggleFeature(message, config, feature, action, prefix) {
        if (!action || (action !== 'on' && action !== 'off')) {
            return message.reply(` Usage: ${prefix}antiraid ${feature} [on/off]`);
        }
    ...
    async configureAntiSpam(message, config, args, prefix) {
        if (!args[0] || (args[0] !== 'on' && args[0] !== 'off')) {
            return message.reply(` Usage: ${prefix}antiraid antispam [on/off] [max_messages] [time_window_ms]`);
        }
    ...
    async configureAntiToken(message, config, args, prefix) {
        if (!args[0] || (args[0] !== 'on' && args[0] !== 'off')) {
            return message.reply(` Usage: ${prefix}antiraid antitoken [on/off] [max_age_days]`);
        }
    ...
    async configureAntiMassMention(message, config, args, prefix) {
        if (!args[0] || (args[0] !== 'on' && args[0] !== 'off')) {
            return message.reply(` Usage: ${prefix}antiraid antimassmention [on/off] [max_mentions]`);
        }
    ...
    async configureAntiCaps(message, config, args, prefix) {
        if (!args[0] || (args[0] !== 'on' && args[0] !== 'off')) {
            return message.reply(` Usage: ${prefix}antiraid anticaps [on/off] [max_percentage]`);
        }
    ...
    async configureAntiBan(message, config, args, prefix) {
        if (!args[0] || (args[0] !== 'on' && args[0] !== 'off')) {
            return message.reply(` Usage: ${prefix}antiraid antiban [on/off] [max_bans] [time_window_ms]`);
        }
    ...
    async manageWhitelist(message, config, args, prefix) {
        const action = args[0]?.toLowerCase();
        
        if (!action || !['add', 'remove', 'list'].includes(action)) {
            return message.reply(` Usage: ${prefix}antiraid whitelist [add/remove/list] [@user]`);
        }
